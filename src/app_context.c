...
/* PathValidator 싱글턴 — static 전역 직접 보관 */
static PathValidator* s_path_validator = NULL;
...
static void AppContext_finalize(Object* obj) {
    ...
    if (s_path_validator) {
        RELEASE((Object*)s_path_validator);
        s_path_validator = NULL;
    }
...
static bool AppContext_init(AppContext* self,
    ...
    /* ── 2. PathValidator 싱글턴 초기화 ── */
    /* _PathValidator_Class 는 static — ServiceRegistry 등록 불가 */
    /* static 전역 s_path_validator 에 직접 보관, 접근자로 제공!! */
    s_path_validator = new_PathValidator();
    if (!s_path_validator) {
    ...
    if (db_host) {
        ...
        /* dbClientClass 는 extern — ServiceRegistry 정상 사용!! */
        self->reg->register_s(self->reg, &dbClientClass, (Object*)db);
        RELEASE((Object*)db);

        if (logger && logger->info) {
            logger->info(logger,
                "[APP] DB connected: %s:%d/%s",
                db_host, db_port, db_name);
        }
    } else {
        if (logger && logger->info) {
            logger->info(logger,
                "[APP] No db_host — DB skipped.");
        }
    }
...
/* ── PathValidator 싱글턴 전역 접근자 ── */
PathValidator* AppContext_getPathValidator(void) {
    return s_path_validator;
}