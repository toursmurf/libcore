...
/* =========================================================
 * 🚨 [STEP 7] V1.5 실전 무선 컨트롤러 시나리오
 * ========================================================= */
void execute_step7_wireless_control(EventLoop* loop) {
    printf("\n[STEP 7] 700대 스위치 무선 컨트롤러 가동 시작...\n");

    HttpClient* client = new_HttpClient(loop);
    if (!client) {
        printf("🚨 HttpClient 초기화 실패!\n");
        return;
    }

    printf("\n🔒 무선 컨트롤러 인증 서버 로그인 시도...\n");
    HashMap* login_data = new_HashMap(4);
    if (!login_data) { RELEASE((Object*)client); return; }

    hashmap_put_str(login_data, "admin_id", "toos_it_admin");
    hashmap_put_str(login_data, "admin_pw", "super_secret_9999");

    HttpClientResponse* login_res = client->POST(client, "https://10.0.0.1/api/login", login_data, PAYLOAD_JSON);
    if (login_res && login_res->status_code == 200) {
        printf("✅ 로그인 성공! (세션 쿠키 자동 저장 완료)\n");
        RELEASE((Object*)login_res);
    } else {
        printf("❌ 로그인 실패! 펌웨어 업데이트를 중단합니다.\n");
        if (login_res) RELEASE((Object*)login_res);
        RELEASE((Object*)login_data);
        RELEASE((Object*)client);
        return;
    }
    RELEASE((Object*)login_data);

    /* 🚨 더미 파일 생성 (stat/fopen 에러 방어) */
    const char* fw_filename = "switch_os_v1.5.0.bin";
    FILE* dummy_fp = fopen(fw_filename, "wb");
    if (dummy_fp) {
        const char* mock_fw_data = "[DUMMY_FIRMWARE_BINARY_HEADER_V1.5.0]";
        fwrite(mock_fw_data, 1, strlen(mock_fw_data), dummy_fp);
        fclose(dummy_fp);
    }

    HashMap* cmd_data = new_HashMap(8);
    if (!cmd_data) { RELEASE((Object*)client); return; }
    hashmap_put_str(cmd_data, "action", "FIRMWARE_UPGRADE");
    hashmap_put_str(cmd_data, "version", "v1.5.0-RELEASE");
    hashmap_put_str(cmd_data, "force_reboot", "true");

    HashMap* upload_files = new_HashMap(4);
    if (!upload_files) { RELEASE((Object*)cmd_data); RELEASE((Object*)client); return; }

    HttpMultipartFile* firmware_file = new_HttpMultipartFile(fw_filename, "application/octet-stream", NULL, 0);

    if (firmware_file) {
        String* key_str = new_String("firmware_blob");
        if (key_str) {
            upload_files->put(upload_files, key_str->c_str(key_str), (Object*)firmware_file);
            RELEASE((Object*)key_str);
        } else {
            RELEASE((Object*)firmware_file);
            RELEASE((Object*)cmd_data);
            RELEASE((Object*)upload_files);
            RELEASE((Object*)client);
            return;
        }
    } else {
    ...
    for (int i = 1; i <= 3; i++) {
        char target_url[256];
        snprintf(target_url, sizeof(target_url), "https://10.0.%d.100/api/system/update", i);

        printf("▶️ [스위치 #%d] 펌웨어 스트리밍 전송 중... (%s)\n", i, target_url);

        HttpClientResponse* res = client->POST_MULTIPART(client, target_url, cmd_data, upload_files);

        if (res) {
            printf("  ✅ 전송 완료! 상태 코드: %d\n", res->status_code);
            if (res->body_len > 0) printf("     응답: %s\n", res->body);
            RELEASE((Object*)res);
        } else {
            printf("  ❌ 전송 실패! (네트워크 오류 또는 파일 누락)\n");
        }
    }
