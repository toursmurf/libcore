...
static void DB_setSaveLog_core(DBClient *self, int enable) { self->save_log = enable; }
...
static void DB_writeLog_core(DBClient *self, const char* msg, const char* sql, int is_error) {
    ...
    char log_file[256];
    snprintf(log_file, sizeof(log_file), "%s/daemon.%04d%02d%02d.log", DEFAULT_LOG_DIR, ti->tm_year + 1900, ti->tm_mon + 1, ti->tm_mday);
    FILE *fp = fopen(log_file, "a+");
    if (fp) {
        fprintf(fp, "%04d-%02d-%02d %02d:%02d:%02d\t[%s]\t[%s@%s:%d]\t[%s]\t%s\t%s\n",
                ti->tm_year + 1900, ti->tm_mon + 1, ti->tm_mday, ti->tm_hour, ti->tm_min, ti->tm_sec,
                self->db_type, self->dbid, self->host, self->port, is_error ? "ERROR" : "INFO", msg, sql ? sql : "");
        fclose(fp);
    }
...
static void DBClient_Finalize(Object *obj) {
    ...
    if (self->isConnected) self->disconnect(self);
...
static void _init_base_methods(DBClient *db) {
    ...
    db->save_log = 0; db->last_insert_id = 0; db->last_idx = 0;
    db->unused_ptr = NULL; db->option_count = 0;
    pthread_mutex_init(&db->lock, NULL);
}

DBClient* new_DBClient() {
    FILE *fp = fopen(DEFAULT_DB_CONFIG, "r");
    if (!fp) return NULL; // 🚀 [의장님 버전] exit(1) 제거 및 NULL 반환

    DBClient *db = calloc(1, sizeof(DBClient));
    Object_Init((Object*)db, &dbClientClass);
    _init_base_methods(db);

    char line[256], val[128]; int idx = 0;
    ...
    fclose(fp);
    if (idx < 7) { RELEASE(db); return NULL; }
    bind_mysql(db);
    return db;
}
